...
export const ChatProvider = ({ children }) => {
    ...
    const fetchSessions = useCallback(async () => {
        try {
            const response = await axios.get('http://127.0.0.1:5000/api/chat/sessions');
    ...
    const fetchHistory = async (sid) => {
        ...
        try {
            const response = await axios.get(`http://127.0.0.1:5000/api/chat/sessions/${sid}`);
    ...
    const createNewSession = async (title = "New Conversation", skipHistoryFetch = false) => {
        try {
            const res = await axios.post('http://127.0.0.1:5000/api/chat/sessions', { title });
    ...
    const deleteSession = async (sid) => {
        try {
            await axios.delete(`http://127.0.0.1:5000/api/chat/sessions/${sid}`);
    ...
    const sendMessage = async (prompt, contextResumeId = null) => {
        ...
        try {
            const response = await axios.post(`http://127.0.0.1:5000/api/chat/sessions/${currentSid}/message`, {
                prompt,
