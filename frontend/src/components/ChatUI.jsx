...
const ChatUI = () => {
    ...
    return (
        <div className="flex flex-col h-[calc(100vh-64px)] w-full theme-transition">
            <div className="flex flex-grow bg-white dark:bg-slate-900 overflow-hidden relative">
                ...
                >
                    <div className="p-4 flex flex-col h-full min-w-[260px]">
                        <div className="flex items-center space-x-2 mb-6 px-1">
                            ...
                            <span className="text-[11px] font-bold text-slate-900 dark:text-slate-100 uppercase tracking-widest">Context Vault</span>
                        </div>
                        ...
                        <div className="flex-grow overflow-y-auto space-y-1.5 pr-1 scrollbar-hide">
                            ...
                            {resumes.map(r => (
                                ...
                                >
                                    ...
                                    <span className="text-[10px] opacity-60 font-mono">ID: {r.id}</span>
                                </button>
                            ))}
                        </div>
                ...
                >
                    {sidebarOpen ? <ChevronLeft className="w-4 h-4 text-slate-500 dark:text-slate-400" /> : <ChevronRight className="w-4 h-4 text-slate-500 dark:text-slate-400" />}
                </button>
                ...
                <div className="flex-grow flex flex-col bg-white dark:bg-slate-900 relative">
                    ...
                    <div className="px-8 py-4 border-b border-slate-100 dark:border-slate-800 flex justify-between items-center bg-slate-50/30 dark:bg-slate-900/50 backdrop-blur-md">
                        <div className="flex items-center gap-3">
                            ...
                            <div>
                                <h3 className="text-sm font-bold text-slate-900 dark:text-white tracking-tight">AI Career Intelligence</h3>
                                <div className="flex items-center gap-1.5 mt-0.5">
                                    <span className="w-1.5 h-1.5 rounded-full bg-emerald-500 animate-pulse" />
                                    <span className="text-[10px] text-slate-400 dark:text-slate-500 font-bold uppercase tracking-widest">Session Active</span>
                                </div>
                            </div>
                        </div>
                    ...
                    >
                        <AnimatePresence initial={false}>
                            {messages.map((msg, idx) => (
                                ...
                                >
                                    <div className={`flex space-x-5 max-w-[80%] ${msg.role === 'user' ? 'flex-row-reverse space-x-reverse' : ''}`}>
                                        ...
                                        <div className="space-y-3">
                                            <div className={`p-5 rounded-[24px] text-[13px] leading-relaxed shadow-sm border theme-transition ${
                                                msg.role === 'user' 
                                                ...
                                                {msg.role === 'assistant' ? (
                                                    <div className="space-y-4">
                                                        {msg.content.split('\n').map((line, lidx) => {
                                                            ...
                                                            return (
                                                                <div key={lidx} className={`${isBullet ? 'flex items-start space-x-3 pl-1' : ''}`}>
                                                                    {isBullet && <span className="text-blue-500 dark:text-blue-400 font-black mt-1.5 shrink-0 scale-125">·</span>}
                                                                    <span className="font-medium">{isBullet ? trimmed.replace(/^[•\-\d+\.]\s*/, '') : trimmed}</span>
                                                                </div>
                                                            );
                                                        })}
                                                    </div>
                    ...
                    <div className="p-6 bg-white dark:bg-slate-900 border-t border-slate-100 dark:border-slate-800 shrink-0">
                        <form onSubmit={handleSend} className="relative group max-w-4xl mx-auto w-full">
                            <input
                                ...
                                placeholder={selectedResumeId === 'global' ? "Ask CareerForge intelligence anything..." : `Query context from ${resumes.find(r => r.id === selectedResumeId)?.filename || 'this profile'}...`}
                        ...
                        <div className="flex items-center justify-center gap-4 mt-4 opacity-50">
                            ...
                            <p className="text-[9px] text-slate-400 dark:text-slate-500 font-black uppercase tracking-[0.2em]">AI Intelligence Cluster</p>
                            <span className="h-px w-8 bg-slate-200 dark:bg-slate-800" />
                        </div>
                    </div>
                </div>
